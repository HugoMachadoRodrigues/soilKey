...
answers a field-realistic question: *given only a photograph of a soil
profile and a GPS fix, what is this soil?* No augered samples, no
laboratory sheet – just what a phone camera and a coordinate can
...
Three things never happen here: the taxonomic key is never delegated to
the model; quantitative non-colour attributes are never read off a
photograph (that is a hard prompt-level rule); and a SoilGrids prior
never silently displaces a measured value – the `PedonRecord` authority
order forbids it.
...
`images` can also be a plain character vector of profile photos when
there is no field sheet:
...
back from canned data by accident would be worse than an error, so you
must supply either a live [ellmer](https://ellmer.tidyverse.org/) chat
object or a `MockVLMProvider` for testing:
...
For each horizon the value is interpolated at the mid-depth from the 0-5
/ 5-15 / 15-30 / 30-60 / 60-100 / 100-200 cm slices and recorded with
`source = "inferred_prior"`. Skip it with `soilgrids = FALSE` if you
only want the Munsell-driven result.

## Reading the evidence grade

Because every value came from a photograph or a prior, the
classification’s evidence grade is low by construction – `D` where a
VLM-extracted attribute was used, `C` where only a SoilGrids prior
contributed. For a cell-by-cell view use
...
The five-grade scale is: **A** measured, **B** spectra-predicted, **C**
prior-inferred, **D** VLM-extracted, **E** user-assumed. A photo-only
classification will never report grade A or B – and that is the point:
the grade tells the user exactly how much trust the result has earned.

## When to use it

[`classify_from_photos()`](https://hugomachadorodrigues.github.io/soilKey/reference/classify_from_photos.md)
is built for reconnaissance: a quick, georeferenced, multi-system first
guess from a field visit, to be confirmed later by description and
sampling. The handful of horizons it can see, the colour it can
estimate, and the regional prior it can fetch are genuinely informative
