...
# soilKey <img src="https://img.shields.io/badge/v0.9.4-pre--CRAN-FF6B35?style=flat-square" align="right"/>
...
[![tests](https://img.shields.io/badge/tests-1694%20expectations-brightgreen.svg?style=flat-square)](tests/)
...
A canonical Brazilian *Latossolo Vermelho Distrocoeso* on Mata Atlântica gneiss, classified end-to-end:
...
That is the **complete WRB 2022 Chapter 6 name** — four principal qualifiers + five supplementary qualifiers in canonical order — produced by deterministic R code driven from versioned YAML rules. The same `PedonRecord` resolves through SiBCS to **Latossolos Vermelhos** and through USDA Soil Taxonomy to **Oxisols** in a single pass.
...
flowchart TB
  ...
  M1 --> W["WRB 2022 key<br/>32 RSGs · Ch 4–6"]
  M1 --> SC["SiBCS 5 key<br/>13 ordens · 44 subordens"]
  M1 --> U["USDA ST 13<br/>12 Orders"]
...
### SiBCS 5ª edição (Embrapa, 2018)

| Categoria               | Coverage |
| :---------------------- | :------- |
| Atributos diagnósticos  | **17**   |
| Horizontes diagnósticos | **24**   |
| Ordens (1º nível categórico) | **13 / 13** |
| Subordens (2º nível)    | **44**   |
...
| :-------------- | :------- |
| Soil Orders     | **12 / 12** |
| Suborders       | scaffold (v1.0+) |
| Great groups    | scaffold (v1.0+) |
...
| Qualifier functions (`qual_*`)    | 139   |
| YAML rules (key + diagnostics + qualifiers) | ~2,250 lines |
| R source                          | ~19,250 lines |
| Test files / expectations         | 44 / 1,694 |
| Vignettes                         | 6     |
| Canonical fixtures                | 31 (one per RSG, plus auxiliaries) |
...
classify_sibcs (pr)$name
#> [1] "Latossolos Vermelhos"

classify_usda  (pr)$rsg_or_order
#> [1] "Oxisols"
```
...
<sub>**Status**: pre-CRAN, v0.9.4. WRB / SiBCS / USDA RSG-level coverage is feature-complete; v1.0 will close the WoSIS benchmark run, the methodology paper, and the CRAN submission. Track the roadmap in [`ARCHITECTURE.md` §12](ARCHITECTURE.md#12-roadmap-de-implementação).</sub>
